...
use crate::json::{
    ...
    game_settings::{GameOptions, UserType},
    install::Callback,
};
    ...
    pub async fn start(&mut self) -> LibResult<()> {
        if self.process.is_some() {
            return Err(LauncherLibError::General(
                "A instance of minecraft is already running".into(),
            ));
        }

        let (java, args) = match get_launch_command(
...
impl ClientBuilder {
    pub async fn install(
        ...
        let mc_dir = if let Some(dir) = minecraft_directory {
            dir
        } else {
    ...
    pub fn new(minecraft_dir: Option<PathBuf>) -> LibResult<Self> {
        let mc_dir = match minecraft_dir {
            Some(value) => value,
            None => match get_minecraft_directory() {
                Ok(value) => value,
                Err(err) => return Err(err),
            },
        };
            ...
            Some(value) => match value {
                Loader::Fabric => {
                    if let Some(lv) = loader_version {
                        self.minecraft =
                            format!("fabric-loader-{loader}-{mc}", loader = lv, mc = minecraft)
                                .to_string();
                    } else {
                ...
                Loader::Forge => {
                    if let Some(lv) = loader_version {
                        self.minecraft =
                            format!("{mc}-forge-{loader}", loader = lv, mc = minecraft).to_string();
                    } else {
                        return Err(LauncherLibError::General("Missing loader version".into()));
                    }
                }
                Loader::Optifine => {
                    if let Some(lv) = loader_version {
                        self.minecraft =
                            format!("{mc}-OptiFine_{loader}", loader = lv, mc = minecraft)
                                .to_string();
                    } else {
                        return Err(LauncherLibError::General("Missing loader version".into()));
                    }
    ...
    pub fn build(&self) -> LibResult<Client> {
        if self.minecraft.is_empty() {
            return Err(LauncherLibError::General(
                "Minecraft version is unset".into(),
            ));
        }
...
mod tests {
    ...
    async fn test_client_builder() {
        use crate::login::{get_auth_code, login_microsoft, ms_login_url};

        let client_id = std::env::var("CLIENT_ID").expect("Failed to get client id");
        let redirect: String =
            "https://login.microsoftonline.com/common/oauth2/nativeclient".into();
